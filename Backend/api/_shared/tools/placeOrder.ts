...
import type { RuntimeCtx } from "./customerInfo.js";

export function buildPlaceOrderTool(ctx: RuntimeCtx) {
  return tool(
    async (args) => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          notes: args.notes ?? `channel:${ctx.channel}; session:${ctx.channel_user_id}`,
          items,
