...
export const lookupCustomerTool = tool(
  async ({ phone, channel, channel_user_id }) => {
    let q = supabase.from("users_info").select("*").limit(1);
    if (phone) q = q.eq("phone", phone);
    else if (channel && channel_user_id)
      q = q.eq("channel", channel).eq("channel_user_id", channel_user_id);
    else return "Refuse: provide phone or (channel + channel_user_id).";
  ...
  },
  {
    ...
    description:
      "Fetch a returning customer's saved delivery details. Use at the start of a conversation, or when the user says 'same address as before'. Try channel+channel_user_id first, fall back to phone.",
    schema: z.object({
      phone: z.string().optional(),
      channel: z.enum(["web", "telegram"]).optional(),
      channel_user_id: z.string().optional(),
    }),
  },
);
